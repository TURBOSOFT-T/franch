...
use App\Models\{commandes,config, User,produits, Category, Service,Marque, Review, Testimonial};
...
class HomeController extends Controller
{
    ...
    public function details($id){
       // $produit =produits::with('attributs')-> findOrFail($id);
       $produit = produits::with(['attributs', 'reviews' => function ($query) {
        $query->where('active', 1)->orderBy('created_at', 'desc');
    }])->findOrFail($id);
    
        $revieews = Review::orderBy('created_at', 'desc')
        ->where('active', '1')
        ->limit(10)->get();
        $configs= config::all();
